  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <h1 className="text-2xl font-black text-stone-900">Experiences</h1>
        ...
        <div className="mb-10">
          ...
          <div className="space-y-3">
            {pending.map((exp: any) => {
              ...
              return (
                <div key={exp.id} className="bg-white border border-orange-100 rounded-2xl p-5 shadow-sm flex flex-wrap items-start justify-between gap-4">
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      <span className="text-lg">{cat?.emoji}</span>
                      <h3 className="font-black text-stone-900">{exp.title}</h3>
                    </div>
                    <div className="text-stone-500 text-sm">
                      {exp.operators?.business_name} · {exp.city} · {exp.duration_hours}h · ${exp.price_per_person}/person
                    </div>
                    <p className="text-stone-400 text-xs mt-1 max-w-lg line-clamp-2">{exp.description}</p>
                  </div>
      ...
      <h2 className="font-bold text-stone-700 text-sm uppercase tracking-wide mb-4">All Experiences ({experiences?.length ?? 0})</h2>
      <div className="bg-white rounded-2xl border border-stone-100 overflow-hidden shadow-sm">
        <table className="w-full text-sm">
          <thead className="bg-stone-50 border-b border-stone-100">
            <tr>
              <th className="text-left px-4 py-3 font-semibold text-stone-600">Title</th>
              <th className="text-left px-4 py-3 font-semibold text-stone-600">Operator</th>
              ...
              <th className="text-center px-4 py-3 font-semibold text-stone-600">Featured</th>
          ...
          <tbody className="divide-y divide-stone-50">
            {experiences?.map((exp: any) => (
              <tr key={exp.id} className="hover:bg-stone-50">
                <td className="px-4 py-3 font-medium text-stone-800 max-w-xs"><p className="line-clamp-1">{exp.title}</p></td>
                <td className="px-4 py-3 text-stone-500">{exp.operators?.business_name}</td>
                <td className="px-4 py-3 text-stone-500">{exp.city}</td>
                ...
                <td className="px-4 py-3 text-center">
                  <form action={toggleExperienceFeaturedAction.bind(null, exp.id, exp.featured)}>
                    <button type="submit" className={`transition-colors ${exp.featured ? "text-amber-500" : "text-stone-200 hover:text-amber-400"}`}>
        ...
        {(!experiences || experiences.length === 0) && (
          <div className="py-12 text-center text-stone-400 text-sm">No experiences yet.</div>
        )}
      </div>
    </div>
  );
